...
#define  LOG_TAG    "ffmpegentry"
#define  LOGI(...)  __android_log_print(ANDROID_LOG_INFO,LOG_TAG,__VA_ARGS__)
#define  LOGE(...)  __android_log_print(ANDROID_LOG_ERROR,LOG_TAG,__VA_ARGS__)
#define  LOGW(...)  __android_log_print(ANDROID_LOG_WARN,LOG_TAG,__VA_ARGS__)
#define  LOGDG(...)  __android_log_print(ANDROID_LOG_DEBUG,LOG_TAG,__VA_ARGS__)
     ...
     if(
      ...
      if (pkt->dts < max) {
        ...
        if(loglevel == AV_LOG_WARNING) {
			...
			LOGDG("Non-monotonous DTS in output stream "
			   "%d:%d; previous: %"PRId64", current: %"PRId64"; ",
			   ost->file_index, ost->st->index, ost->last_mux_dts, pkt->dts);
		}
        if (exit_on_error) {
        ...
        if(loglevel == AV_LOG_WARNING) {
			...
			LOGDG("changing to %"PRId64". This may result "
					"in incorrect timestamps in the output file.\n",
					max);
		}
        if(pkt->pts >= pkt->dts)
        ...
        if (pkt->dts != AV_NOPTS_VALUE &&
            ...
            pkt->dts > pkt->pts) {
            LOGW("Invalid DTS: %"PRId64" PTS: %"PRId64" in output stream %d:%d\n",
                   pkt->dts, pkt->pts,
                         ...
                         AVFrame *in_picture)
{
    ...
    case VSYNC_VSCFR:
        if (ost->frame_number == 0 && delta - duration >= 0.5) {
            LOGDG("Not duplicating %d initial frames\n", (int)lrintf(delta - duration));
            delta = duration;
  ...
  for (i = 0; i < nb_frames; i++) {
    ...
    if (s->oformat->flags & AVFMT_RAWPICTURE &&
        ...
        if (forced_keyframe) {
            ...
            LOGDG("Forced keyframe at time %f\n", pts_time);
        }
        ...
        while (1) {
            ...
            if (ret < 0) {
                if (ret != AVERROR(EAGAIN) && ret != AVERROR_EOF) {
                    LOGW("Error in av_buffersink_get_frame_flags(): %s\n", av_err2str(ret));
                }
                break;
            }
            if (ost->finished) {
            ...
            case AVMEDIA_TYPE_AUDIO:
                ...
                if (!(enc->codec->capabilities & CODEC_CAP_PARAM_CHANGE) &&
                    ...
                    LOGE("Audio filter graph output is not normalized and encoder does not support parameter changes\n");
                    break;
                }
...
static int decode_video(InputStream *ist, AVPacket *pkt, int *got_output)
{
    ...
    for (i = 0; i < ist->nb_filters; i++) {
        ...
        if (ret == AVERROR_EOF) {
            ...
            LOGE("Failed to inject frame into filter network: %s\n", av_err2str(ret));
            exit_program(1);
        }
    }
    ...
    if (ist->fix_sub_duration) {
        ...
        if (ist->prev_sub.got_output) {
            ...
            if (end < ist->prev_sub.subtitle.end_display_time) {
                LOGDG("Subtitle duration reduced from %d to %d%s\n",
...
static int output_packet(InputStream *ist, const AVPacket *pkt)
{
    ...
    handle_eof:
        ...
        if (avpkt.size && avpkt.size != pkt->size &&
        	...
        	if(ist->showed_multi_packet_warning) {
        		LOGI("Multiple frames in a packet from stream %d\n", pkt->stream_index);
        	} else {
        		LOGW("Multiple frames in a packet from stream %d\n", pkt->stream_index);
        	}
    ...
    for (p = pix_fmts; *p != -1; p++) {
        ...
        if (ret < 0) {
            if (ist->hwaccel_id == hwaccel->id) {
                LOGE("%s hwaccel requested for input stream #%d:%d, "
    ...
    for (i = 0; i < nb_output_streams; i++) {
        ...
        if (ost->encoding_needed) {
            ...
            if (av_opt_set_dict(ost->enc_ctx, &ost->encoder_opts) < 0) {
               LOGE("Error setting up codec context options.\n");
                exit_program(1);
            }
        }
...
static int check_keyboard_interaction(int64_t cur_time)
{
    ...
    if (key == 'c' || key == 'C'){
        ...
        if (k > 0 &&
            ...
            LOGDG("Processing command target:%s time:%f command:%s arg:%s",
