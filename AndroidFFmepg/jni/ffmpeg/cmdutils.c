...
{
	LOGW("Exit program++++++++++++++++++>>>>>>>>>>>>>>>>>>>>>>>T_T ret=>>%d", ret);
    if (program_exit)
        program_exit(ret);
...
int parse_optgroup(void *optctx, OptionGroup *g)
{
    ...
    LOGDG( "Parsing a group of options: %s %s.\n",
    ...
    for (i = 0; i < g->nb_opts; i++) {
        ...
        LOGDG( "Applying option %s (%s) with argument %s.\n",
    ...
    LOGDG( "Successfully parsed a group of options.\n");
                      ...
                      const OptionGroupDef *groups, int nb_groups)
{
    ...
    LOGDG( "Splitting the commandline.\n");
...
do {                                                                           \
    ...
    if (!arg) {                                                                \
        ...
        if ((ret = match_group_separator(groups, nb_groups, opt)) >= 0) {
            ...
            LOGDG( " matched as %s with argument '%s'.\n",
                   groups[ret].name, arg);
            continue;
        }
        ...
        if (po->name) {
            ...
            LOGDG( " matched as option '%s' (%s) with "
                   "argument '%s'.\n", po->name, po->help, arg);
            continue;
        }
        ...
        if (argv[optindex]) {
            ...
            if (ret >= 0) {
                LOGDG( " matched as AVOption '%s' with "
                       "argument '%s'.\n", opt, argv[optindex]);
                optindex++;
                continue;
    ...
    LOGDG( "Finished splitting the commandline.\n");
...
#define PRINT_LIB_INFO(libname, LIBNAME, flags, level)                  \
    if (CONFIG_##LIBNAME) {                                             \
        ...
        if (flags & SHOW_VERSION) {                                     \
            ...
            LOGW("%slib%-11s %2d.%3d.%3d / %2d.%3d.%3d\n",            \
                   indent, #libname,                                    \
