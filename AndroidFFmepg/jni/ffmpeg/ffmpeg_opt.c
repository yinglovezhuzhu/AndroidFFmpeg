...
//#define  LOGDG(...)  av_log(NULL, AV_LOG_DEBUG,__VA_ARGS__)
#include <android/log.h>
                      ...
                      int (*open_file)(OptionsContext*, const char*))
{
    ...
    for (i = 0; i < l->nb_groups; i++) {
        ...
        LOGW("parse_optgroup in open_files() line:2651===>>ret=>>%d", ret);
        if (ret < 0) {
        ...
        LOGDG( "Opening an %s file: %s.\n", inout, g->arg);
        ret = open_file(&o, g->arg);
        LOGW("parse_optgroup in open_file() line:2651===>>ret=>>%d", ret);
        uninit_options(&o);
        if (ret < 0) {
        ...
        LOGDG( "Successfully opened the file.\n");
    }
...
int ffmpeg_parse_options(int argc, char **argv)
{
    ...
    LOGW("split_commandline in ffmpeg_parse_options() line:2682 ret++>>%d", ret);
    if (ret < 0) {
    ...
    LOGW("parse_optgroup in ffmpeg_parse_options() line:2690 ret++>>%d", ret);
    if (ret < 0) {
    ...
    LOGW("parse_optgroup in open_files()**input_files** line:2698 ret++>>%d", ret);
    if (ret < 0) {
    ...
    LOGW("parse_optgroup in open_files()**output_files** line:2698 ret++>>%d", ret);
    if (ret < 0) {
